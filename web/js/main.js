function init() {
    var app = new Vue({
        el: '#devkit',
        data: {
            functionAppName: 'device-state-functionapp-dw',
            deviceName: 'iot-mxchip',
            reportedTwin: {
                wifi: {},
                sensor: {}
            },
...
function getTwin() {
    ...
    var url = `https://${this.functionAppName}.azurewebsites.net/api/devkit-state?action=get&deviceName=${this.deviceName}&t=${timespan}`;
...
function updateUserLEDState() {
    ...
    this.stateUpdateEndpoint = `https://${this.functionAppName}.azurewebsites.net/api/devkit-state?action=set&state=${state}&key=userLED&deviceName=${this.deviceName}&t=${timespan}`;
...
function updateRgbLEDState() {
    ...
    this.stateUpdateEndpoint = `https://${this.functionAppName}.azurewebsites.net/api/devkit-state?action=set&state=${state}&key=rgbLED&deviceName=${this.deviceName}&t=${timespan}`;
    setTimeout(this.getTwin, 1000);
...
function updateRgbLEDColor() {
    ...
    this.stateUpdateEndpoint = `https://${this.functionAppName}.azurewebsites.net/api/devkit-state?action=set&state=${state.r},${state.g},${state.b}&key=rgbLEDColor&deviceName=${this.deviceName}&t=${timespan}`;
    setTimeout(this.getTwin, 1000);
}
